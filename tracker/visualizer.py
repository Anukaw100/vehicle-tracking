# Inspired by the Detectron2 Visualizer utilities at
# "https://github.com/facebookresearch/detectron2/tree/master/detectron2/utils".
import numpy
...
class CustomVisualizer:
    ...
    def draw_panoptic_seg_predictions(
        ...
        masks_rles = mask_util.encode(
            numpy.asarray(numpy.asarray(masks).transpose(1, 2, 0), dtype=numpy.uint8, order="F")
        )
        assert len(masks_rles) == num_instances
    ...
    def _assign_colors(self, instances):
        ...
        is_crowd = numpy.zeros((len(instances),), dtype=numpy.bool)
        if instances[0].bbox is None:
            ...
            threshold = 0.6
        if len(ious) == 0:
            ious = numpy.zeros((len(self._old_instances), len(instances)), dtype="float32")
        ...
        matched_new_per_old = numpy.asarray(ious).argmax(axis=1)
        max_iou_per_old = numpy.asarray(ious).max(axis=1)
